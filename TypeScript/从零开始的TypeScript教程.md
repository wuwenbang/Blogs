...
`TypeScript` 是一款由微软开发的静态类型编程语言，它是具有**类型语法**的 `JavaScript`，并且 `TypeScript` 还提供最新的和不断发展的 `JavaScript` 特性，最终 `TypeScript` 会编译成 `JavaScirpt`。

简单归纳一下就是：`TypeScript` = `Type` + `JavaScript`。

## 1.2 为什么要学 TypeScript

### 1.2.1 静态类型

众所周知，`JavaScript` 是一门解释型语言，而解释型语言一般是**动态类型**的，这种语言的类型错误往往会导致运行时错误。<br>
下图是 `JavaScript` 项目中最常见的十大错误：
![JavaScript 十大错误](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5daeeb0a8664434b9716f2e11a73edda~tplv-k3u1fbpfcp-watermark.awebp)
而 `TypeScript` 是一门**静态类型编译型语言**，其在运行前需要先编译为 `JavaScript`，在编译阶段 `TypeScript` 能确定每个变量的类型，进行类型检查。所以如果使用 `TypeScript` ，那么在编写阶段就可以发现并解决很多 `JavaScript` 错误了。

### 1.2.2 支持最新的 JavaScript 功能

`TypeScript` 始终紧跟 `ECMAScript` 标准，支持 ES6/7/8/9 等新语法标准。并且，在语法层面上对一些语法进行了扩展，例如可选链表达式`?.`，双问号表达式`??` 等。

### 1.2.3 开发工具支持

`TypeScript` 配合开发工具如 `VSCode` 可以获得极其舒适的开发体验：
语法高亮、类型提示、自动补全、静态错误提示等等。
